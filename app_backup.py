...
# Applied State Keys (von DB-Funktion gelesen) - Initialisierung auf UI-Werte
def _init_applied_state():
    """Initialisiert die 'applied_'-Keys mit den aktuellen UI-Werten beim ersten Lauf."""
    keys_to_copy = ['zeitraum_input', 'start_date_input', 'end_date_input', 'produkt_filter_exklusiv', 'kunde_input',
                    'produkt_input']
    for key in keys_to_copy:
        applied_key = f'applied_{key}'
...
if st.session_state.get('data_applied', False):
    ...
    if df_eventlog.empty:
        st.warning(
            "Es konnten keine Eventlog-Daten geladen werden (aufgrund restriktiver Filter).")
